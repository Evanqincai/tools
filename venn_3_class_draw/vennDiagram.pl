...
&USAGE unless ($file[0] and $file[1]);
$file[2]||="None";
$file[3]||="None";
$file[4]||="None";
$out_dir||=".";
...
## draw venn picture
	...
	my $read_table="data<-read.table(\"$R_file\",header=F,sep=\"\\t\")"."\n";
	...
	my $out_pic1="filename=\"venn.png\"";
	my $out_pic2="filename=\"venn.svg\"";
	my $cat_dist=($sample_num==3)?'cat.dist=c(0.12,0.12,0.07)':'';
	my $venn_command="venn.diagram($list,$margin,$height,$width,$resolution,$units,$lwd,$fill,$cex,$cat_cex,$scaled,$cat_pos,$category_name,$out_pic1,$cat_dist,imagetype=\"png\")"."\n";
	my $svg="venn.diagram($list,$margin,$height/200,$width/200,$resolution,$units,$lwd,$fill,$cex,$cat_cex,$scaled,$cat_pos,$category_name,$out_pic2,$cat_dist,imagetype=\"svg\")"."\n";
    open OUT,">","venn.R";
	...
	close OUT;
	chdir($out_dir);
	`Rscript venn.R &&cp $Bin/readme.txt $out_dir`;
## sub functions
## read_file
sub read_list
{
